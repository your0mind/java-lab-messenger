...
public class SignInRequestHandler implements RequestHandler<SignInRequest> {
    @Override
    public Response handle(SignInRequest request, ServerThread callerThread,
        ...
        if (user == null) {
            return new SignInResponse(new ErrorMessage("Unknown username"));
        } else if (!params.getPassword().equals(user.getPassword())) {
            return new SignInResponse(new ErrorMessage("Invalid password"));
        } else {
